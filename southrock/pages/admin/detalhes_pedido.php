...
// $link_configuracoes_admin = 'configuracoes.php'; // Descomente se tiver

$nome_sistema_atual = "SouthRock Admin"; // Nome estático por enquanto
...
$query = "SELECT p.*, 
          ...
          u_origem.endereco AS endereco_origem, 
          u_origem.cidade AS cidade_origem, 
          u_origem.uf AS uf_origem,
...
if (!$stmt) {
    $_SESSION['error_message_pedidos'] = "Erro ao preparar consulta do pedido: " . $conn->error;
    header("Location: pedidos.php");
    exit();
}
...
$query_itens = "SELECT i.*, pr.produto, pr.unidade_medida, i.tipo_item_troca
                FROM pedido_itens i
...
if ($pedido['tipo_pedido'] === 'troca') {
    foreach ($itens_pedido_array as $item_adm) {
        if ($item_adm['tipo_item_troca'] === 'enviado') {
            $items_enviados_admin[] = $item_adm;
        } elseif ($item_adm['tipo_item_troca'] === 'recebido') {
            $items_recebidos_admin[] = $item_adm;
        }
    }
} else {
...
if ($pedido['filial_destino_id']) {
    ...
    if (in_array($pedido['tipo_pedido'], ['requisicao', 'devolucao'])) {
        $nome_destino_final = "Matriz";
    }
}

$statusInfo = [
    'novo' => ['bg' => '#E3F2FD', 'text' => '#1565C0', 'icon' => 'fa-file-circle-plus', 'label' => 'Novo'],
    'processo' => ['bg' => '#FFF8E1', 'text' => '#F57F17', 'icon' => 'fa-spinner fa-spin', 'label' => 'Em Processo'],
    'finalizado' => ['bg' => '#E8F5E9', 'text' => '#2E7D32', 'icon' => 'fa-circle-check', 'label' => 'Finalizado'],
    'aprovado' => ['bg' => '#D1E7DD', 'text' => '#0A3622', 'icon' => 'fa-thumbs-up', 'label' => 'Aprovado'],
    'rejeitado' => ['bg' => '#F8D7DA', 'text' => '#58151C', 'icon' => 'fa-times-circle', 'label' => 'Rejeitado'],
    'cancelado' => ['bg' => '#E2E3E5', 'text' => '#41464B', 'icon' => 'fa-ban', 'label' => 'Cancelado'],
    'novo_troca_pendente_aceite_parceiro' => ['bg' => '#FFF3CD', 'text' => '#664D03', 'icon' => 'fa-hourglass-start', 'label' => 'Troca: Aguardando Parceiro'],
    'troca_aceita_parceiro_pendente_matriz' => ['bg' => '#CFF4FC', 'text' => '#055160', 'icon' => 'fa-handshake', 'label' => 'Troca: Aceita pelo Parceiro']
];
$tipoPedidoInfo = [
...
$currentStatus = $statusInfo[$pedido['status']] ?? ['bg' => '#F8F9FA', 'text' => '#212529', 'icon' => 'fa-question-circle', 'label' => ucfirst(str_replace('_', ' ', $pedido['status']))];
$tipoPedido = $tipoPedidoInfo[$pedido['tipo_pedido']] ?? ['icon' => 'fa-question-circle', 'label' => ucfirst($pedido['tipo_pedido'])];

function hex2rgba_admin_details($color, $opacity = false) {
...
<head>
    ...
    <?php
        // Inclui o header com o menu lateral (que pode usar seu próprio jQuery e Bootstrap)
    ...
    <style>
        ...
        body.hcm-body-fixed-header { /* Esta classe é adicionada pelo header_com_menu */
            background-color: var(--light-bg);
            color: var(--dark-text);
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            /* padding-top já é gerenciado pelo header_com_menu.css, não precisa redefinir aqui */
        }
        ...
        @media print {
            body.hcm-body-fixed-header { background-color: white; padding-top: 0 !important; }
            .hcm-navbar, .hcm-sidebar { display: none !important; } 
        ...
        .alert-dismissible .btn-close { /* Para Bootstrap 5 */
            padding: 0.75rem 1rem; box-sizing: content-box;
        }
         .badge.p-2 { padding: 0.5rem .75rem !important; }
    </style>
</head>
<body class="hcm-body-fixed-header">
        ...
        <div class="container py-4">
            ...
            <?php if(isset($_SESSION['error_message_pedidos'])): ?>
                 <div class="alert alert-danger alert-dismissible fade show" role="alert">
                    <?= htmlspecialchars($_SESSION['error_message_pedidos']); unset($_SESSION['error_message_pedidos']); ?>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
                ...
                <div class="order-main-card">
                    <div class="order-header">
                        <div class="order-id-container">
                            <div class="order-id">
                                <div class="type-icon-wrapper" style="background-color: <?= htmlspecialchars(hex2rgba_admin_details($currentStatus['text'] ?? '#000000', 0.1)) ?>;">
                                    <i class="fas <?= htmlspecialchars($tipoPedido['icon']) ?> type-icon" style="color: <?= htmlspecialchars($currentStatus['text'] ?? '#000000') ?>;"></i>
                                </div>
                                <div>
                     ...
                     <div class="progress-container">
                        <?php
                        $progressWidthPercent = 0; $dataCriado = $pedido['data'];
                        $dataIntermediaria = $pedido['data_processamento'] ?? null; $dataFinalizacao = $pedido['data_finalizacao'] ?? null;
                        $stepCriadoLabel = "Criado"; $stepMeioLabel = "Processamento";
                        $stepCriadoClass = 'completed'; $stepMeioClass = 'pending'; $stepFinalizadoClass = 'pending';
                        $iconMeio = 'fa-hourglass-half'; $iconFinalizado = 'fa-flag-checkered';
                        switch($pedido['status']) {
                            case 'novo': $progressWidthPercent = 5; break;
                            case 'novo_troca_pendente_aceite_parceiro':
                                $stepCriadoLabel = "Proposta Enviada"; $stepMeioLabel = "Aceite Parceiro";
                                $progressWidthPercent = 25; $iconMeio = 'fa-handshake'; break;
                            case 'aprovado': $stepCriadoLabel = "Aprovado Matriz"; $progressWidthPercent = 33; break;
                            case 'troca_aceita_parceiro_pendente_matriz':
                                $stepCriadoLabel = "Proposta Enviada"; $stepMeioLabel = "Parceiro Aceitou";
                                $dataIntermediaria = $pedido['data_atualizacao']; $stepMeioClass = 'completed';
                                $iconMeio = 'fa-check'; $progressWidthPercent = 66;
                                $stepFinalizadoClass = 'pending'; $iconFinalizado = 'fa-cogs'; break;
                            case 'processo':
                                $progressWidthPercent = ($pedido['tipo_pedido'] === 'troca') ? 80 : 50;
                                $stepMeioClass = 'active'; $iconMeio = 'fa-spinner fa-spin'; break;
                            case 'finalizado':
                                $progressWidthPercent = 100; $stepMeioClass = 'completed'; $iconMeio = 'fa-check';
                                $stepFinalizadoClass = 'completed'; $iconFinalizado = 'fa-check'; break;
                            case 'rejeitado': case 'cancelado':
                                $iconMeio = 'fa-times'; $iconFinalizado = 'fa-times';
                    ...
                    <div class="content-section">
                        ...
                        <div class="filial-cards">
                            <div class="filial-card">
                                ...
                                <div class="info-grid">
                                    <div class="info-item"><div class="info-label">Nome</div><div class="info-value"><?= $nome_origem_final ?></div></div>
                                    ...
                                    <?php endif; ?>
                                    <div class="info-item"><div class="info-label">Endereço</div><div class="info-value"><?= htmlspecialchars($pedido['endereco_origem'] ?: 'N/A') ?></div></div>
                                    <div class="info-item"><div class="info-label">Cidade/UF</div><div class="info-value"><?= htmlspecialchars($pedido['cidade_origem'] ?: 'N/A') ?>/<?= htmlspecialchars($pedido['uf_origem'] ?: 'N/A') ?></div></div>
                                </div>
                            </div>
                            ...
                            <div class="filial-card">
                                ...
                                <div class="info-grid">
                                    <div class="info-item"><div class="info-label">Nome</div><div class="info-value"><?= $nome_destino_final ?></div></div>
                                    ...
                                    <?php endif; ?>
                                    <div class="info-item"><div class="info-label">Endereço</div><div class="info-value"><?= htmlspecialchars($pedido['endereco_destino'] ?: 'N/A') ?></div></div>
                                    <div class="info-item"><div class="info-label">Cidade/UF</div><div class="info-value"><?= htmlspecialchars($pedido['cidade_destino'] ?: 'N/A') ?>/<?= htmlspecialchars($pedido['uf_destino'] ?: 'N/A') ?></div></div>
                                </div>
                            </div>
                            <?php endif; ?>
                        </div>
                                ...
                                <div class="col-lg-6 mb-4 mb-lg-0">
                                    ...
                                    <div class="table-responsive">
                                        <table class="items-table">
                                            <thead><tr><th>SKU</th><th>Produto</th><th>Qtd. (Und)</th><th>Obs.</th></tr></thead>
                                            <tbody>
                        ...
                        <?php else: // Para outros tipos de pedido ?>
                            ...
                            <div class="table-responsive">
                                <table class="items-table">
                                    <thead>
                                        <tr>
                                            <th style="width: 15%">SKU</th><th style="width: 30%">Produto</th>
                                            <th style="width: 15%">Quantidade</th><th style="width: 15%">Unidade</th>
                                    ...
                                    <tbody>
                                        <?php if (!empty($items_normais_admin)): ?>
                                            ...
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr><td colspan="5" class="text-center" style="padding: 20px;">Nenhum item encontrado para este pedido.</td></tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="content-section">
                        <div class="order-actions">
                            ...
                            <?php if (isset($_SESSION['tipo_usuario']) && $_SESSION['tipo_usuario'] == 1): // Admin actions ?>

                                <?php if ($pedido['tipo_pedido'] === 'troca'): ?>
                                    <?php // Ações Específicas para TROCA ?>
                                ...
                                <?php else: // Ações para OUTROS TIPOS DE PEDIDO (requisicao, doacao, devolucao) ?>
                                    ...
                                    <?php endif; ?>
                                <?php endif; // End if $pedido['tipo_pedido'] === 'troca' ?>

                                <?php // Ações GENÉRICAS (Editar, Cancelar, Rejeitar para não-trocas) ?>
                                <?php if (!in_array($pedido['status'], ['finalizado', 'cancelado'])): ?>
                                    <?php if ($pedido['status'] !== 'rejeitado' ): // Se não estiver já rejeitado (para evitar duplicar info de rejeição) ?>
                                    ...
                                    <?php // Admin pode Rejeitar diretamente apenas se NÃO for uma troca e estiver em estado inicial
    ...
    <script>
    function hex2rgba_admin_details(color, opacity = 1) {
        if (!color || color.indexOf('#') !== 0) return `rgba(0,0,0,${opacity})`; 
        const hex = color.replace('#', '');
        if (hex.length !== 3 && hex.length !== 6) return `rgba(0,0,0,${opacity})`;
        let r, g, b;
        if (hex.length === 3) {
